...
public class OrderDAOImpl extends BaseDAO<OrderBean> implements OrderDAO {
    ...
    public void addOrderBean(OrderBean orderBean) {
        int orderBeanId = super.executeUpdate("insert into t_order values(0,?,?,?,?,?,?)",orderBean.getOrderNo(),orderBean.getOrderDate(),orderBean.getOrderUser().getId(),orderBean.getOrderMoney(),orderBean.getOrderStatus(),orderBean.getTotalBookCount());
    ...
    public Integer getOrderTotalBookCount(OrderBean orderBean) {
        String sql = "SELECT SUM(t3.buyCount) AS totalBookCount , t3.orderBean FROM " +
                "(" +
                "SELECT t1.id , t2.buyCount , t2.orderBean FROM t_order t1 INNER JOIN t_order_detail t2 " +
...
//              String sql ="select sum(buyCount)  from t_order_detail  group by orderBean having orderBean=?";
//              return ((BigDecimal)executeComplexQuery(sql,orderBean)[0]).intValue();
